...
/* cursor position should be within the boundary of the window */
int check_cursor_position(WINDOW* wnd, int c_x, int c_y)
{
    return (c_x >= 0) && (c_x < wnd->width) && (c_y >= 0) && (c_y < wnd->height);
}

void move_cursor(WINDOW* wnd, int x, int y)
{
    if(check_cursor_position(wnd, x, y))
		wnd->cursor_x = x;
		wnd->cursor_y = y;
}
...
{
    int i, src_line_addr, des_line_addr;
    des_line_addr = get_window_start_addr(wnd);
    src_line_addr = des_line_addr + LINE_MEMORY_CAPACITY;
    for(i = 1; i < wnd->height; i++)
		copy_line(src_line_addr, des_line_addr, wnd->width);
		des_line_addr = src_line_addr;
		src_line_addr += LINE_MEMORY_CAPACITY;
	for(i = 0; i < wnd->width; i++) // clear the last line content
		poke_w_char(src_line_addr + i * 2, ' ');
}
...
void output_char(WINDOW* wnd, unsigned char c)
{
	...
	wnd->cursor_x++;
    if(should_move_to_next_line(wnd, c))
		move_to_next_line(wnd);
}
